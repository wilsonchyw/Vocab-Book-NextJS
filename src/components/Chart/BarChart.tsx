...
function BarChart({ dateMap }: { dateMap: dateMap }): FunctionComponent {
    const [startDay, setStart] = useState<Date>(new Date());
    ...
    const initChartData = (): { labels: string[]; datasets: object } => {
        const labels: Date[] = Array(weekly ? 7 : 30)
            .fill(null)
            .map((_, index) => new Date(startDay - DAY * index))
            .reverse();

        return {
            labels: labels.map((label: Date) => label.toLocaleDateString().replace(/-/g, "/")),
            datasets: DATASET_NAME.map((name, index) => ({
                label: name.charAt(0).toLocaleUpperCase() + name.slice(1),
                data: labels.map((date) => dateMap[name].get(date.toDateString())),
    ...
    const dayShift = (offset: number) => {
        if (startDay >= new Date() && offset > 0) return;
        const day = new Date(startDay.getTime() + offset * DAY);
        console.log("day", startDay, day, offset, DAY);
        setStart(day);
    };
            ...
            <Row className="text-white" style={{ zIndex: 99 }}>
                ...
                <Col>
                    <ButtonGroup className="d-flex align-items-center">
                        <Button variant={weekly ? "secondary" : "outline-secondary"} onClick={() => setWeekly(true)}>
                            Weekly
                        </Button>
                        <Button variant={!weekly ? "secondary" : "outline-secondary"} onClick={() => setWeekly(false)}>
                            Monthly
                        </Button>
                    </ButtonGroup>
                </Col>
