...
export const dialogSlice = createSlice({
    ...
    reducers: {
        ...
        setEdit: (state: dialogState, action: PayloadAction<{ vocab?: Vocab; inflection?: Inflection }>) => {
            if (action.payload.vocab) state.vocab = action.payload.vocab;
            if (action.payload.inflection) {
                state.inflection = typeof action.payload.inflection === "string" ? JSON.parse(action.payload.inflection) : action.payload.inflection;
            } else {
                state.inflection = initialState.inflection;
            }
            if (action.payload.example) {
                state.example = typeof action.payload.example === "string" ? JSON.parse(action.payload.example) : action.payload.example;
            } else {
                state.example = [];
            }
            state.show = true;
        ...
        setExample: (state: dialogState, action: PayloadAction<{ index: number; value: string }>) => {
...
export const { handleHide, handleShow, setEdit, toggleCreate, changeVocab, setExample, setInflection } = dialogSlice.actions;
