        ...
        <div className="px-1">
            ...
            {example.map((sentence) => (
                <Row key={sentence} className="my-2">
                    <Col style={{ maxWidth: "24px" }} onClick={() => speak(sentence)}>
                        <PlayCircleIcon />
                    </Col>
                    <Col onClick={() => speak(sentence)}>{sentence}</Col>
                </Row>
            ))}
        </div>
    ) : null;
...
export const Footer: FunctionComponent = ({ inflection, example }: Footer) => {
    const inflectionComponent = inflection.hasContent ? (
        <div>
            ...
            {Object.keys(inflection.content)
                .filter((key) => !!inflection.content[key].vocab)
                .map((key: string) => (
                    <Row key={key} className="px-3 my-2">
                        <Col style={{ maxWidth: "80px" }} className=" p-0" onClick={() => speak(inflection.content[key].vocab)}>
                            <PlayCircleIcon />
                        </Col>
                        ...
                        <Col className="px-0">{inflection.content[key].meaning}</Col>
            ...
            {Object.keys(inflection.content)
                ...
                .map((key: string) => (
                    <Row key={key} className="px-2">
                        <Col xs={5}>{inflection.content[key].vocab}</Col>
                        <Col xs={5} className="p-0">
                            {inflection.content[key].meaning}
                        </Col>
                        <Col xs={2}>{key === "adjective" ? "adj" : key}</Col>
                    </Row>
                ))}
        </div>
    ) : null;
    return (
        <div>
            ...
            <Row className="px-2">
                ...
                <Col xs={1} className="px-0" onClick={() => speak(data.vocabulary)}>
                    <PlayCircleIcon  />
                </Col>
            </Row>
            <Example example={example} />
